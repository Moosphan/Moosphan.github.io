...
| 能否不发生跳转 | 不能 | 不能 | 能 | 能 |
| 能否去下载页面 | 能 | 能 | 不能 | 能 |
| iframe 触发 | 不支持 | 不支持 | Chrome 
1 
<= 18, iOS < 9 | 不支持 |
| 链接格式 
2 | 正常的 URL | 正常的 URL | 自定义协议的 URL | intent 协议的 URL |
...
```
**[scheme]://[host]/[path]?[params]**` 例如：`http://www.baidu.com/data?page=home&uid=ut9092
```
